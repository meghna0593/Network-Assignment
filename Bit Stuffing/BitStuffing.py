...
def bitStuffing(binValue):
    stuffedBit = re.sub(r'(\s*1\s*1\s*1\s*1\s*1\s*)', r'\g<1>0',
                        binValue)  # can also use just \1 to match the grouped string.
    return stuffedBit


def bitUnstuffing(binValue):
    unstuffedBit = re.sub(r'(\s*1\s*1\s*1\s*1\s*1\s*)(0)', r'\g<1>' + re.escape(""),
                          binValue)  # removing second group and keeping the first
    return unstuffedBit
...
if __name__ == "__main__":
    ...
    stuffedbitStream = stuffedBitStream.replace(" ", "")
    displayValues("The bit stream after bitstuffing:", stuffedbitStream)
    ...
    if (strValue == text):
        ...
        fw.write("Bit stream after bit stuffing: "+stuffedbitStream)
        fw.write("\n")
        fw.write("String after removing bitstuffing: "+strValue)
        fw.close()
